...
import {
  ...
  useTelegramStartMutation,
  useTelegramStopMutation,
  useTelegramRestartMutation,
} from "@/hooks/useTelegramBot"

type Props = { channelId: string }

export function TelegramBotControl({ channelId }: Props) {
  const { data: statuses = [], isLoading } = useTelegramStatusQuery()
  const startMutation = useTelegramStartMutation()
  const stopMutation = useTelegramStopMutation()
  const restartMutation = useTelegramRestartMutation()
