...
export function getAgentTeamColumns({
  ...
  return [
    ...
    },
    {
      id: "lead",
      header: "Lead Agent",
      cell: ({ row }) => agentMap.get(row.original.leadAgentId)?.name ?? row.original.leadAgentId,
    },
    {
      id: "members",
      header: "Members",
      cell: ({ row }) => (
        <span className="text-muted-foreground">{row.original.members?.length ?? 0} agents</span>
      ),
    },
    {
      id: "orchestration",
      ...
      cell: ({ row }) => (
        <Badge variant={row.original.autoOrchestration ? "secondary" : "outline"}>
          {row.original.autoOrchestration ? "auto" : "manual"}
        </Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-right">
          <Button asChild size="sm" variant="outline">
            <Link to="/agent-teams/$agentTeamId/edit" params={{ agentTeamId: row.original.id }}>
