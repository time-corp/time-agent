...
const parseImageErrorMessage = async (response: Response) => {
  ...
  try {
    if (contentType.includes("application/json")) {
      ...
      return payload.error?.message ?? payload.message ?? `Image generation failed: ${response.status}`
    }
...
export const generateImageForAgent = async ({
  ...
  if (!provider.apiKey) {
    throw new AppError(ErrorCode.VALIDATION_ERROR, "Provider API key is missing", 422)
  }
  ...
  const attachments = (payload.data ?? [])
    .filter((item): item is { url: string; mime_type?: string } => typeof item.url === "string" && item.url.length > 0)
    .map((item) => ({
      type: "image" as const,
      url: item.url,
  ...
  if (attachments.length === 0) {
    throw new AppError(ErrorCode.INTERNAL_ERROR, "Image generation returned no images", 502)
  }
